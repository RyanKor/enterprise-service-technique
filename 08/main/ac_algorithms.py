...
class AhoCorasick:
    def __init__(self, words: List[str]):
        ...
        self.out: List[int] = [0] * (self.max_states + 1)
        ...
        self.fail: List[int] = [-1] * (self.max_states + 1)
        ...
        # It has been initialized to all -1.
        self.goto: List[List[int]] = [[-1] * self.max_characters for _ in range(self.max_states + 1)]
        ...
        self.states_count: int = self.__build_matching_machine()

    # Builds the String matching machine.
    # Returns the number of states that the built machine has.
    # States are numbered 0 up to the return value - 1, inclusive.
    def __build_matching_machine(self) -> int:
        k: int = len(self.words)
        ...
        for i in range(k):
            ...
            for character in word:
                ch: int = ord(character) - 97  # Ascii value of 'a' = 97
                ...
                current_state:int = self.goto[current_state][ch]
    ...
    def __find_next_state(self, current_state: int, next_input: str) -> int:
        answer: int = current_state
        ch: int = ord(next_input) - 97  # Ascii value of 'a' is 97, 띄어쓰기를 넣으면 에러 발생
    ...
    def search_words(self, text: str) -> dict:
...
if __name__ == "__main__":
    words: List[str] = ["he", "she", "hers", "his"]
    text: str = "ahishogehershe"

    # Create an Object to initialize the Trie
    aho_chorasick: AhoCorasick = AhoCorasick(words)

    # Get the result
    result: dict = aho_chorasick.search_words(text)
